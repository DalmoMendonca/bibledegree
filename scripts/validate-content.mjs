...
for(const name of fs.readdirSync('public/data/lectures')){const id=name.replace('.json','');assert(ids.has(id),id);const s=JSON.parse(fs.readFileSync('public/data/lectures/'+name));assert(s.transcript?.length>1000 || /^https:\/\//.test(s.transcriptUrl||''));assert(s.sourceUrl&&s.attribution);}
for(const name of fs.readdirSync('public/data/guides')){const g=JSON.parse(fs.readFileSync('public/data/guides/'+name));assert(ids.has(name.replace('.json','')));assert(g.summary&&g.sourceNote);assert.equal(g.basis,'transcript');assert(g.mastery.length>=6&&g.reflection.length>=4);assert(g.quiz.length>=12);for(const q of g.quiz){assert(q.options.length===4);assert(new Set(q.options).size===4);assert(Number.isInteger(q.answer)&&q.answer>=0&&q.answer<4);assert(q.explanation.length>30);}assert(new Set(g.quiz.map(q=>q.answer)).size===4);}
