...
function strip(html: string){return html.replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<[^>]+>/g,' ').replace(/&amp;/g,'&').replace(/&#39;/g,"'").replace(/&quot;/g,'\"').replace(/\s+/g,' ').trim()}
function uniq(items: any[]){const seen=new Set();return items.filter(x=>{const k=x.videoId||x.url||x.title;if(!k||seen.has(k))return false;seen.add(k);return true})}
...
async function youtube(url: URL){
  ...
  return {kind:'youtube',title:'YouTube course',lessons:uniq(lessons)};
}
...
async function webpage(url: URL){
  ...
  while((m=anchor.exec(html))){const text=strip(m[2]);if(!text)continue;const numbered=text.match(/^\s*(\d{1,3})[.)]?\s+(.{3,160})$/);if(!numbered)continue;try{const href=new URL(m[1],r.url).toString();lessons.push({title:text.replace(/^\s*\d{1,3}[.)]?\s*/,''),url:href})}catch{}}
  if(!lessons.length){
  ...
  return {kind:'web',title,lessons:uniq(lessons).slice(0,150)};
}

export default async (req: Request) => {
  try{
    const u=new URL(req.url).searchParams.get('url');if(!u)return Response.json({error:'Missing url'},{status:400});
    const url=new URL(u);if(!ALLOWED.has(url.hostname))return Response.json({error:'Source host not allowed'},{status:400});
  ...
  }catch(e){return Response.json({error:'Could not read source course'},{status:502})}
}
